...
class JanusTraceApp(ctk.CTk):
    ...
    def build_scan_ui(self):
        ...
        # Config File
        ctk.CTkLabel(form_frame, text="Config File (.yaml):").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.entry_config = ctk.CTkEntry(form_frame, width=300)
        self.entry_config.grid(row=0, column=1, padx=10, pady=10)
        ...
        btn_browse_cfg.grid(row=0, column=2, padx=10)
        ToolTip(btn_browse_cfg, "Select the YAML configuration file.")

        # Reqs File
        ctk.CTkLabel(form_frame, text="Requirements (.csv/xlsx):").grid(row=1, column=0, padx=10, pady=10, sticky="w")
        self.entry_reqs = ctk.CTkEntry(form_frame, width=300)
        self.entry_reqs.grid(row=1, column=1, padx=10, pady=10)
        ...
        btn_browse_req.grid(row=1, column=2, padx=10)
        ToolTip(btn_browse_req, "Select the Requirements file (CSV or Excel).")

        # Source Dir
        ctk.CTkLabel(form_frame, text="Source Directory:").grid(row=2, column=0, padx=10, pady=10, sticky="w")
        self.entry_source = ctk.CTkEntry(form_frame, width=300)
        self.entry_source.grid(row=2, column=1, padx=10, pady=10)
        btn_browse_src = ctk.CTkButton(form_frame, text="Browse", width=80, command=lambda: self.browse_file(self.entry_source, "dir"), fg_color=THEME_PRIMARY, text_color=THEME_TEXT_ON_PRIMARY, hover_color=THEME_PRIMARY_HOVER)
        btn_browse_src.grid(row=2, column=2, padx=10)
        ToolTip(btn_browse_src, "Select the root folder of your source code.")

        # Output Dir
        ctk.CTkLabel(form_frame, text="Output Directory:").grid(row=3, column=0, padx=10, pady=10, sticky="w")
        ...
        btn_browse_out.grid(row=3, column=2, padx=10)
        ToolTip(btn_browse_out, "Select where the HTML report will be saved.")

        # Action
        self.btn_run = ctk.CTkButton(self.scan_frame, text="START SCAN", height=40, font=ctk.CTkFont(size=16, weight="bold"), command=self.run_scan_thread, fg_color=THEME_PRIMARY, text_color=THEME_TEXT_ON_PRIMARY, hover_color=THEME_PRIMARY_HOVER)
        ...
        self.log(f"Ready to scan. Please select files.\nCWD: {os.getcwd()}")
    ...
    def run_logic(self):
        try:
            ...
            results = engine.link(reqs, all_traces)
...
2. Config File: Select your configuration YAML file (e.g., config/default_rules.yaml).
3. Requirements: Select one or more CSV/Excel files containing your requirements.
4. Source Directory: Select the root folder of your source code.
5. Output Directory: Select where the reports will be saved.
6. Click START SCAN.
